...
func _physics_process(delta):
	print(current_direction, next_direction)
	...
	match next_direction:
		'left':
			if not (left_1.is_colliding() or left_2.is_colliding()):
				current_direction = next_direction
		'right':
			if not (right_1.is_colliding() or right_2.is_colliding()):
				current_direction = next_direction
		'up':
			if not (up_1.is_colliding() or up_2.is_colliding()):
				current_direction = next_direction
		'down':
			if not (down_1.is_colliding() or down_2.is_colliding()):
				current_direction = next_direction
